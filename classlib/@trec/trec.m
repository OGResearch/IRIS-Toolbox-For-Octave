classdef trec < getsetobj
    % trec  Time-Recursive Subscript Objects for Time-Recursive Expressions.
    %
    ...
    % indication of an error) when the following two circumstances occur at the
    % same time:
    %
    ...
    % or function to the tseries `d.y` before it enters the time-recursive
    % expression. Below are examples of some simple manipulations that do the
    % job without changing the tseries `d.y`:
    %
    %     d = struct();
    %     d.x = tseries(1:10,1);
    %     d.y = 1*d.x;
