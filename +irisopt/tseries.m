...
Def.acf = { ...
    'demean',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'order',0,@(varargin)is.numericscalar(varargin{:}), ...
    'smallsample',true,@(varargin)is.logicalscalar(varargin{:}), ...
    };

Def.bpass = { ...
    'addtrend',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'detrend',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'log',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'method','cf',@(x) ischar(x) && any(strcmpi(x,{'cf','hwfsf'})), ...
    'ttrend',[],@(x) isempty(x) || is.logicalscalar(x), ...
    'unitroot',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'window','hamming',@(x) ischar(x) && any(strcmpi(x,{'hamming','hanning','none'})), ...
    };

Def.chowlin = { ...
    'constant',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'log',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'ngrid',200,@(x) is.numericscalar(x) && x >= 1, ...
    'rho','estimate', ...
    @(x) any(strcmpi(x,{'auto','estimate','negative','positive'})) ...
    || (is.numericscalar(x) && x > -1 && x < 1), ...
    'timetrend',false,@(varargin)is.logicalscalar(varargin{:}), ...
    };
...
Def.fft = { ...
    'full',false,@(varargin)is.logicalscalar(varargin{:}), ...
    };
...
Def.myplot = { ...
    ...
    'tight',false,@(varargin)is.logicalscalar(varargin{:}), ...
...
Def.moving = { ...
    ...
    'function',@mean,@(varargin)is.func(varargin{:}), ...
    };

Def.barcon = {
    'barwidth',0.8,@(varargin)is.numericscalar(varargin{:}), ...
    'colormap',[],@isnumeric, ...
    'evenlyspread',true,@(varargin)is.logicalscalar(varargin{:}), ...
...
Def.plotpred = { ...
    'connect',true,@(varargin)is.logicalscalar(varargin{:}), ...
    ...
    'shownanlines',true,@(varargin)is.logicalscalar(varargin{:}), ...
};

Def.plotyy = { ...
    Def.myplot{:}, ...
    'coincident',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'highlight',[],@isnumeric, ...
    'lhsplotfunc',@plot,@(x) ischar(x) || is.func(x), ...
    'rhsplotfunc',@plot,@(x) ischar(x) || is.func(x), ...
    'lhstight',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'rhstight',false,@(varargin)is.logicalscalar(varargin{:}), ...
    };
...
Def.trend = { ...
    'break',[],@isnumeric, ...
    'connect',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'diff',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'log',false,@(varargin)is.logicalscalar(varargin{:}), ...
...
Def.x12 = { ...
    ...
    'cleanup,deletetempfiles,deletetempfile,deletex12file,deletex12file,delete',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'dummy',[],@(x) isempty(x) || isa(x,'tseries'), ...
    'dummytype','holiday',@(x) ischar(x) && any(strcmpi(x,{'holiday','td','ao'})), ...
    'display',false,@(varargin)is.logicalscalar(varargin{:}), ...
    ...
    'missing',false,@(varargin)is.logicalscalar(varargin{:}), ...
    ...
    'tdays,tday',false,@(varargin)is.logicalscalar(varargin{:}), ...
