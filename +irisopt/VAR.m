function Def = VAR()
% VAR  [Not a public function] Default options for VAR class functions.
%
% Backend IRIS function.
% No help provided.

% -IRIS Toolbox.
% -Copyright (c) 2007-2014 IRIS Solutions Team.

%--------------------------------------------------------------------------

Def = struct();

outputFmt = { ...
    'output','auto',@(x) any(strcmpi(x,{'auto','dbase','tseries','array'})), ...
    };

applyFilter = { ...
    'applyto',Inf,@(x) isnumeric(x) || islogical(x) || isequal(x,':') || iscellstr(x), ...
    'filter','',@ischar, ...
    };

tolerance = { ...
    'tolerance',getrealsmall(),@(varargin)is.numericscalar(varargin{:}), ...
    };

output = { ...
    'output','namedmat',@(x) ischar(x) && any(strcmpi(x,{'namedmat','numeric'})), ...
    };

Def.acf = { ...
    applyFilter{:}, ...
    output{:}, ...
    'nfreq',256,@(varargin)is.numericscalar(varargin{:}), ...
    'order',0,@(varargin)is.numericscalar(varargin{:}), ...
    'progress',false,@(varargin)is.logicalscalar(varargin{:}), ...
    }; %#ok<*CCAT>

Def.demean = { ...
   };

Def.estimate = [ ...
    outputFmt, { ...
    'a',[],@isnumeric, ...
    'bvar',[],@(x) isempty(x) || isa(x,'BVAR.bvarobj'), ...
    'c',[],@isnumeric, ...    
    'j',[],@isnumeric, ...
    'diff',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'g',[],@isnumeric, ...
    'order',1,@(x) isnumeric(x) && numel(1) == 1, ...
    'cointeg',[],@isnumeric, ...
    'comment','',@(x) ischar(x) || isequal(x,Inf), ...
    'constraints,constraint','',@(x) ischar(x) || iscellstr(x) || isnumeric(x), ...
    'constant,const,constants',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'covparameters,covparameter,covparam',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'eqtnbyeqtn',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'maxiter',1,@(varargin)is.numericscalar(varargin{:}), ...
    'mean',[],@(x) isempty(x) || isnumeric(x), ...
    'progress',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'schur',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'stdize',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'tolerance',1e-5,@(varargin)is.numericscalar(varargin{:}), ...
    'timeweights',[],@(x) isempty(x) || isa(x,'tseries'), ...
    'ynames,yname',{},@iscellstr, ...
    'warning',true,@(varargin)is.logicalscalar(varargin{:}), ...
    ... Panel VARs
    'fixedeff,fixedeffect',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'groupweights',[],@(x) isempty(x) || isnumeric(x), ...
    }];

Def.filter = { ...
    'ahead',1,@(x) isnumeric(x) || x == round(x) || x >= 1, ...
    'cross',true,@(x) is.logicalscalar(x) || (is.numericscalar(x) && x >=0 && x <= 1), ...
    'deviation,deviations',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'meanonly',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'omega',[],@isnumeric, ...
    'output','smooth',@ischar, ...    
    };

Def.fmse = { ...
    output{:}, ...
    }; %#ok<CCAT1>

Def.forecast = { ...
    outputFmt{:},  ...
    'cross',true,@(x) is.logicalscalar(x) || (is.numericscalar(x) && x >=0 && x <= 1), ...
    'dboverlay,dbextend',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'deviation,deviations',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'meanonly',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'omega',[],@isnumeric, ...
    'returninstruments,returninstrument',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'returnresiduals,returnresidual',true,@(varargin)is.logicalscalar(varargin{:}), ...
    };

Def.integrate = { ...
    'applyto',Inf,@(x) isnumeric(x) || islogical(x), ...
    };

Def.isexplosive = [ ...
    tolerance, ...
    ];

Def.isstationary = [ ...
    tolerance, ...
    ];


Def.portest = { ...
    'level',0.05,@(x) is.numericscalar(x) && x > 0 && x < 1, ...
    };

Def.resample = { ...
    outputFmt{:}, ...
    'deviation,deviations',false,@(varargin)is.logicalscalar(varargin{:}), ...   
    'method','montecarlo',@(x) is.func(x) ...
    || (ischar(x) && any(strcmpi(x,{'montecarlo','bootstrap'}))), ...
    'progress',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'randomise,randomize',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'wild',false,@(varargin)is.logicalscalar(varargin{:}), ...
    };

Def.simulate = { ...
    outputFmt{:}, ...
    'contributions,contribution',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'deviation,deviations',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'returnresiduals,returnresidual',true,@(varargin)is.logicalscalar(varargin{:}), ...
    };

Def.sprintf = { ...
    'constant,constants,const',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'decimal',[], @(x) isempty(x) || is.numericscalar(x), ...
    'declare',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'enames,ename',[],@(x) isempty(x) || iscellstr(x) || is.func(x), ...
    'format','%+.16g',@ischar, ...
    'hardparameters,hardparameter',true,@(varargin)is.logicalscalar(varargin{:}), ...
    'tolerance',getrealsmall(),@(varargin)is.numericscalar(varargin{:}), ...
    'ynames,yname',[],@(x) isempty(x) || iscellstr(x), ...
    };

Def.response = { ...
    'presample',false,@(varargin)is.logicalscalar(varargin{:}), ...
    'select',Inf,@(x) isequal(x,Inf) || islogical(x) || isnumeric(x) || ischar(x) || iscellstr(x), ...
    };

Def.VAR = { ...
    'exogenous',{},@(x) ischar(x) || iscellstr(x), ...
    };

Def.xsf = { ...
    applyFilter{:}, ...
    'progress',false,@(varargin)is.logicalscalar(varargin{:}), ...
    };

end